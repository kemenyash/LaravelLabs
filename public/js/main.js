let currentCarId = ''; // Store the ID of the car being edited
        ...
        if (data.status === 200 && data.data) {
            ...
            cars.forEach(car => {
                ...
                row.innerHTML = `
                    ...
                    <td>${car.color}</td>
                    <td>
                        <a id="js-modal-confirm" class="uk-button uk-button-default" href="#" uk-toggle="target: #edit-modal" onclick="openEditModal('${car.id}', '${car.owner}', '${car.brand}', '${car.license_plate}', '${car.color}')">edit</a>
                        <a href="/api/cars?brand=${car.brand}&license_plate=${car.license_plate}" target="_blank" class="uk-button uk-button-default">data set</a>
                    </td>
                `;
...
function openEditModal(id, owner, brand, licensePlate, color) {
    currentCarId = id; 
    document.getElementById("editCode").value = id;  // updated field to display the car ID
...
async function updateCar(event) {
    ...
    var id = document.getElementById("editCode").value;
    ...
    try {
        const response = await fetch(`/api/cars/${id}`, { // change endpoint to /api/cars/{id}
            method: 'PUT', 
            headers: {
